...
// connection, videos on Wi-Fi only, and nothing for local/optimistic paths.
// The actual cache warming is a side effect; only the decision is unit-tested.
...
void main() {
  group('MediaPrefetch.decide', () {
    ...
    test('a remote video prefetches only on Wi-Fi', () {
      expect(
        MediaPrefetch.decide(
      ...
      for (final net in ['cellular', 'none', 'unknown']) {
        expect(
          MediaPrefetch.decide(
            file: 'https://cdn.example/v.mp4',
            mediaType: 'video',
          ...
          PrefetchAction.none,
          reason: 'videos are heavy — never prefetch off Wi-Fi ($net)',
        );
      }
    });
